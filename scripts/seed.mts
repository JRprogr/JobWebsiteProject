...
for (const c of companies) {
  await db.query(
    `insert into companies (name, slug, sector, source_type, source_config, active, careers_url, hq_country, logo_url)
    ...
    [c.name, c.slug, c.sector, c.source_type, JSON.stringify(c.source_config), c.active, c.careers_url ?? null, c.hq_country ?? null, c.logo_url ?? null],
  );
