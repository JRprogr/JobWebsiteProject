...
- **cornerstone** (OHB, GMV; `{host, corp, site?}`): the careers page embeds a bearer token, which the site's own `rec-job-search/external/jobs` service accepts (100 per page, description inline). `stripCss()` removes a vendor stylesheet that GMV's descriptions come polluted with.
...
- Tried in Phase 5 and NOT added: Gate Space and Gradel (Cloudflare/403 for Node's TLS fingerprint, bot wall), SpaceOpal (expired TLS certificate), NanoAvionics (times out), Aldoria (only an email address), LookUp Space (WTTJ, loaded by script), Odysseus/OQ Technology/Adwaiseo/ecaps (script-rendered, no data in the HTML), Anywaves, Bradford (no listings), Antwerp Space, Skyrora, Needronix, OroraTech, GovSat, Sirius (page shows no open roles), EnduroSat (no list), Intuitive Machines (ADP, US only, paged API), Rheinmetall, CSEM and Honeywell (no careers URL). Companies with 0 roles are simply not in the register until they post something.
