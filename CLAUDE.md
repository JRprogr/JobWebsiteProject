...
- Light theme map (later iteration): terminal colour `#fb5503`, map field `--map-panel: #b1b5b9`, the whole card tinted grey (`--map-card-bg`, class `map-card`), and a faint glow on the dots (`--map-glow`, drop-shadow, in the theme's own colour) plus a weaker one on the map card's heading, readout, buttons and switch (`--map-ui-glow`, rule `.map-card h2/p/button`); both themes.
