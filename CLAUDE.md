...
- Tried in Phase 5 and NOT scrapable (now listed in the register only, see the last section; EnduroSat and Intuitive Machines were built afterwards): Gate Space and Gradel (Cloudflare/403 for Node's TLS fingerprint, bot wall), Rheinmetall (too heavy), OQ Technology, Odysseus Space, adwaisEO, ECAPS and LookUp Space (script-rendered), Anywaves, Bradford, Antwerp Space, Skyrora, Needronix, OroraTech, GovSat and Sirius (no open roles or no list). Still neither scraped nor listed: Aldoria (only an email address), NanoAvionics (times out) and SpaceOpal (expired TLS certificate).
