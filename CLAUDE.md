...
- Light theme map (current try; grey card, orange and vibrant red were tried and dropped): glassy white card, sky blue terminal `#2aa8f2` (`--map*` tokens; text in it `rgb(3 105 161)` for contrast), pink country codes (`--map-ticker: #d40f74`), conservative glow on the dots (`--map-glow`) and on the card's heading, readout, buttons and switch (`--map-ui-glow`, rule `.map-card h2/p/button`), and a glow around the map's inner frame (`--map-frame-glow`, both themes; green in the dark one).
