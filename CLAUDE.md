...
- Light theme map (final iteration; grey card and orange were tried and dropped): glassy white card, vibrant terminal red `#f5002a` (`--map*` tokens), pink country codes (`--map-ticker: #d40f74`), stronger glow on the dots (`--map-glow`), a weaker one on the card's heading, readout, buttons and switch (`--map-ui-glow`, rule `.map-card h2/p/button`) and a glow around the map's inner frame (`--map-frame-glow`). The dark theme has the same glow tokens in green (frame glow off) and was left as it was.
