...
## Reliability (Batch 4 of the pre-deploy plan)
- `app/error.tsx` (route-level boundary, brings `TopBar` only because the footer is an async server component that asks the database) and `app/global-error.tsx` (replaces the whole document, own inline stylesheet, follows the OS colour scheme). In Next 16.3 the props are `error` and `retry` (`unstable_retry` before 16.3, `reset` only clears state). A failing server render answers 500 with a shell, the boundary paints in the browser, so `curl` alone does not show the error page.
- `/api/health` (`app/api/health/route.ts`): 200 `ok` when the database answers and the last successful scrape finished within 180 minutes, otherwise 503 `stale` or `down` (never cached; the healthy answer is cached 30 s). `.github/workflows/monitor.yml` (hourly at :47, variable `SITE_URL`) fails after three tries and skips a site answering `x-vercel-error: DEPLOYMENT_PAUSED`. `robots.txt` disallows `/api/`.
- Known gap: GitHub disables scheduled workflows of a public repository after 60 days without commits, silencing scrape, backup and monitor together. The README recommends an independent uptime monitor on `/api/health`.

## Security and cost safety (security batch)
- **No invoice is possible without a payment method** (Vercel Hobby, Neon Free, public repo; verified in the providers' docs 2026-09-26): overuse blocks features (Hobby 30 days, Neon compute suspends), it never bills. Never make the repository private (Actions minutes) and never add a payment method or an AI/metered API without the user deciding it. No AI SDK, API key or third-party request exists in the code.
- `next.config.ts`: security headers on every response (CSP with `'unsafe-inline'` for scripts and styles because Next and the theme script inline, `frame-ancestors 'none'`, nosniff, referrer, permissions, COOP), `poweredByHeader: false`, and `Cache-Control: public, max-age=0, s-maxage=300, stale-while-revalidate=1800` on `/`, `/companies`, `/statistics` (a custom Cache-Control from the config survives on these dynamic pages; verified with `next start`). The CSP was tested in a browser with a job selected and its listing dialog open, zero violations; the dev server adds `'unsafe-eval'` (React refresh), production does not, so an eval-based test script only works against `next dev`.
- The database is only ever queried with placeholders; the few interpolated pieces (`ORDER[f.sort]`, `limit`, column lists) are constants or clamped numbers; every query parameter is length- and format-limited (`app/page.tsx`, search words 40 chars).
- Optional read-only web role: `scripts/predeploy.mts` uses `DATABASE_ADMIN_URL` for the deploy's migrate/seed when set, so `DATABASE_URL` in Vercel can be a `pg_read_all_data` role created with SQL (README has the steps; roles made with the Neon console's "Add role" are `neon_superuser` members and can write).
...
- Dark theme yellow is `#feda49` (was `#ffcc00`; also in the icons and the share image). The light theme's map colours are in the `--map*` tokens of `app/globals.css` (see the last light-theme note below).
...
- Light theme map (final; grey/orange, vibrant red and sky blue were tried and dropped): glassy white card, the original terminal red `#d4001f` (`--map*` tokens), yellow country codes `--map-ticker: #ffd21f` with a dark red outline (`--map-ticker-outline`, needed to read on white; none in the dark theme), a conservative glow on the dots (`--map-glow`) and on the card's heading, readout, buttons and switch (`--map-ui-glow`, rule `.map-card h2/p/button`), and a glow around the map's inner frame (`--map-frame-glow`, red in light, green in dark).
