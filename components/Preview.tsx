...
export function Preview({ job, onClose }: { job: JobView | null; onClose: () => void }) {
  const wrapper = `max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-40 max-md:rounded-b-none max-md:pb-[max(1rem,env(safe-area-inset-bottom))] ${
    job ? "" : "max-md:hidden"
  }`;
  ...
  return (
    <section aria-label="Role preview" className={`glass flex flex-col gap-4 rounded-[22px] p-5 md:p-[22px] ${wrapper}`}>
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <h2 className="text-lg font-bold leading-tight md:text-xl">{job.title}</h2>
        ...
        <button type="button" onClick={onClose} aria-label="Close preview" className="grid size-9 shrink-0 place-items-center rounded-full border border-line">
          <CloseIcon />
        </button>
      </div>

      <div className="h-px bg-line max-md:hidden" />
      <dl className="grid grid-cols-3 gap-3 max-md:hidden">
      ...
      <dl className="grid grid-cols-2 gap-3 max-md:hidden">
